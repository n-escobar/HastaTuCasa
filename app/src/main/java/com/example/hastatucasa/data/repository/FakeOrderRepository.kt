...
/**
 * Shopper-scoped order repository.
 *
 * Now delegates storage to [SharedOrderStore] so that orders placed by the
 * shopper are immediately visible in the deliverer flavor (same process).
 */
@Singleton
class FakeOrderRepository @Inject constructor(
    private val store: SharedOrderStore,
) : OrderRepository {

    override fun observeOrdersForShopper(shopperId: String): Flow<List<Order>> =
        store.orders.map { list ->
            list.filter { it.shopperId == shopperId }
                .sortedByDescending { it.createdAt }
        }

    override suspend fun getOrder(orderId: String): Order? =
        store.orders.value.find { it.orderId == orderId }
    ...
    override suspend fun updateOrderStatus(
        ...
        val current = store.orders.value.find { it.orderId == orderId }
